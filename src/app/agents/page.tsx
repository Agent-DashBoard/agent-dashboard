...
function AgentCard({ name, description, status, id }: Agent) {
  const statusConfig = {
    online: { color: "bg-green-500", text: "Online", badgeClass: "bg-green-500/20 text-green-400" },
    offline: { color: "bg-zinc-500", text: "Offline", badgeClass: "bg-zinc-500/20 text-zinc-400" },
    busy: { color: "bg-yellow-500", text: "Busy", badgeClass: "bg-yellow-500/20 text-yellow-400" },
  };

  const currentStatus = statusConfig[status];

  return (
    <div className="relative group rounded-lg border border-zinc-800 bg-zinc-900/50 p-6 hover:border-zinc-700 hover:bg-zinc-900/80 transition-all duration-200 flex flex-col justify-between h-full">
      {/* Header */}
