// NOTE PENANDA: CreateSkillModal_v1.0
// Modal untuk membuat Skill baru
...
export function CreateSkillModal({ isOpen, onClose, onSkillCreated }: CreateSkillModalProps) {
  ...
  const [category, setCategory] = useState<Skill['category']>('coding'); // Default category
  const [status, setStatus] = useState<Skill['status']>('active'); // Default status
  const [icon, setIcon] = useState<string>(''); // For icon name
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (createdSkill) {
        ...
        setError('Gagal membuat skill baru.');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Terjadi kesalahan saat membuat skill.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="relative w-full max-w-md rounded-lg border border-zinc-800 bg-zinc-900 p-6 shadow-lg">
        ...
        <h2 className="mb-6 text-2xl font-bold text-white">Buat Skill Baru</h2>
